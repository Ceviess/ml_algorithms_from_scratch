            ...
            for i in range(len(score_sorted) - 1):
                ...
                for j in range(i + 1, len(score_sorted)):
                    if score_sorted[j][0] == 1 and score_sorted[j][1] == score_sorted[i][1]:
                        ...
                        ranked += 1
            return ranked / np.sum(np.where(y_true == 1, 1, 0)) / np.sum(np.where(y_true == 0, 1, 0))
                    
        
    def calc_loss(self, y_true, y_pred):
        eps = 1e-15
        if self.reg == 'l1':
            return -np.mean(y_true * np.log(y_pred + eps) + (1 - y_true) * np.log(1 - y_pred + eps)) + self.l1 * np.sum(np.abs(self.weights))
        elif self.reg == 'l2':
            return -np.mean(y_true * np.log(y_pred + eps) + (1 - y_true) * np.log(1 - y_pred + eps)) + self.l2 * np.sum(np.square(self.weights))
        elif self.reg == 'elasticnet':
            return -np.mean(y_true * np.log(y_pred + eps) + (1 - y_true) * np.log(1 - y_pred + eps)) + self.l1 * np.sum(np.abs(self.weights)) + self.l2 * np.sum(np.square(self.weights))
        else:
            return -np.mean(y_true * np.log(y_pred + eps) + (1 - y_true) * np.log(1 - y_pred + eps))
    
    def calc_grad(self, X, y_true, y_pred):
        if self.reg == 'l1':
            grad = np.dot(np.transpose(y_pred - y_true), X) / X.shape[0] + self.l1 * np.sign(self.weights)
        elif self.reg == 'l2':
            grad = np.dot(np.transpose(y_pred - y_true), X) / X.shape[0] + self.l2 * 2 * self.weights
        elif self.reg == 'elasticnet':
            grad = np.dot(np.transpose(y_pred - y_true), X) / X.shape[0] + self.l1 * np.sign(self.weights) + self.l2 * 2 * self.weights
        else:

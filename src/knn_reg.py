...
class MyKNNReg:
    ...
    def cosine(self, row1, row2):
        return 1 - (
            np.sum(row1 * row2)
            / (np.sqrt(np.sum(np.square(row1))) * np.sqrt(np.sum(np.square(row2))))
        )
    ...
    def calc_rank_weights(self, neighbors_idxs):
        ...
        for idx, row in enumerate(neighbors_idxs):
            y = self.y.loc[row]
            result.append(
                np.sum([(1 / (i + 1)) * val / denom for i, val in enumerate(y)])
            )
        return result
    ...
    def calc_distance_weights(self, neighbors_idxs):
        ...
        for idx, neighbors in enumerate(neighbors_idxs):
            ...
            result.append(
                np.sum([((1 / distances[i]) * val) / denom for i, val in enumerate(y)])
            )
        return result
    ...
    def predict(self, X):
        ...
        idx_train = 0
        self.dist_matrix = [
            [float("inf") for i in range(self.X.shape[0])] for j in range(X.shape[0])
        ]

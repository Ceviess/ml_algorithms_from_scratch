...
class MyLineReg:
    ...
    def calc_loss(self, y, predictions):
        if not self.reg:
            ...
            loss = np.mean(np.square(predictions - y)) + self.l1 * np.sum(
                np.abs(self.weights)
            )
        elif self.reg == "l2":
            loss = np.mean(np.square(predictions - y)) + self.l2 * np.sum(
                np.square(self.weights)
            )
        else:
            loss = (
                np.mean(np.square(predictions - y))
    ...
    def calc_grad(self, X, y, predictions):
        ...
        if not self.reg:
            ...
            grad = 2 / n_rows * np.dot(
                np.transpose(predictions - y), X
            ) + self.l1 * np.sign(self.weights)
        elif self.reg == "l2":
            grad = (
                2 / n_rows * np.dot(np.transpose(predictions - y), X)

class MyKNNClf():
    ...
    def calc_cosine(self, arr1, arr2):
        return 1 - (np.sum([x*y for x,y in zip(arr1, arr2)])) / np.sqrt(np.sum(np.square(arr1)) * np.sum(np.square(arr2)))
        
    def get_nearest_neighbors(self, X_test):
        self.distance_matrix = [[float('inf') for i in range(self.X.shape[0])] for j in range(X_test.shape[0])]
        test_idx = 0
        train_idx = 0
        for _, test_row in X_test.iterrows():
            for _, train_row in self.X.iterrows():
                self.distance_matrix[test_idx][train_idx] = self.calc_distance[self.metric](test_row, train_row)
                train_idx += 1
            test_idx += 1
            train_idx = 0
        nearest_neighbors = [sorted(zip(row, self.y), key=lambda x: x[0])[:self.k] for row in self.distance_matrix]
    ...
    def calc_rank_weights(self, nearest_neighbors):
        ...
        for row in nearest_neighbors:
            ...
            first_class_weight = np.sum(1 / (np.argwhere(target == 1).squeeze() + 1)) / np.sum(1 / np.arange(1, len(target) + 1))
            second_class_weight = np.sum(1 / (np.argwhere(target == 0).squeeze() + 1)) / np.sum(1 / np.arange(1, len(target) + 1))
    ...
    def calc_distance_weights(self, nearest_neighbors):
        ...
        for row in nearest_neighbors:
            ...
            target = np.array([y for distance, y in row])
            first_class_weight = np.sum(1 / distance[np.argwhere(target == 1)].squeeze()) / np.sum(1 / distance)
            second_class_weight = np.sum(1 / distance[np.argwhere(target == 0)].squeeze()) / np.sum(1 / distance)
    ...
    def calc_uniform(self, nearest_neighbors):
        ...
        for row in nearest_neighbors:
            ...
            predicted_class = 1 if np.sum(target) >= (len(target) - np.sum(target)) else 0
